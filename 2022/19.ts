...
function parseInput(input: string) {
  const blueprintDescriptions = input.split("\n")
    .map(s => s.trim())
    .filter(s => s !== "")
...
function getPossibleStates(oldState: FactoryState, blueprint: Blueprint) {
  ...
  for (const resource of RESOURCES) {
    ...
    if (RESOURCES.every((neededResource) => oldState.resources[neededResource] >= cost[neededResource])) {
...
function hasExcessRobots(state: FactoryState, blueprint: Blueprint) {
  if (state.robots.ore > Math.max(blueprint.ore.ore, blueprint.clay.ore, blueprint.obsidian.ore, blueprint.geode.ore)) return true
...
function estimateGeodes(state: FactoryState, untilMinute: number) {
  ...
  return state.resources.geode + (state.robots.geode + Math.floor(remainingMinutes / 2)) * remainingMinutes
}
...
function part1(input: string) {
  ...
  const max = blueprints.map(b => calculateMaxGeodes(b, 24))
    .map((v, i) => v * (i + 1))
    .reduce((a, b) => a + b, 0)
...
function calculateMaxGeodes(blueprint: Blueprint, time: number) {
  ...
  while (stack.length > 0) {
    ...
    if (state.minutes < time && estimateGeodes(state, time) > maxGeodes && !hasExcessRobots(state, blueprint)) {
  ...
  console.log(`Finished with ${maxGeodes} geodes (${visited.size} states checked)`)
  return maxGeodes
}
